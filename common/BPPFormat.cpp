...
static void CreateResourceInfoVector(int32 length, uint8 * resources, vector<ResourceInfo *> & rInfos);
static void DeleteResourceInfoVector(vector<ResourceInfo *> & rInfos);
static int32 RemoveResources(int32 resourceLength,
const vector<ResourceInfo *> & rInfos, uint8 * resourcePtr);
static int32 RemoveResource(uint32 type, uint16 id, uint8 * imageResource, int32 size, uint8 * dest);
...
DLLExport MACPASCAL void PluginMain (const int16 selector, FormatRecordPtr formatParamBlock, intptr_t * data, int16 * result) {
  ...
  try {
    ...
    //  the parameter block points to an AboutRecord, which is used on Windows.
    //---------------------------------------------------------------------------
    if (selector == formatSelectorAbout) {
      ...
      if (gCountResources == NULL || gGetResources == NULL || gAddResource == NULL || gFormatRecord->advanceState == NULL) {
    ...
    if (selector == formatSelectorAbout || selector == formatSelectorWriteFinish || selector == formatSelectorReadFinish || selector == formatSelectorOptionsFinish || selector == formatSelectorEstimateFinish || selector == formatSelectorFilterFile || *gResult != noErr) {
      PIUSuitesRelease();
    }
...
static int CheckIdentifier (char identifier []) {
  ...
  if (identifier[0] == 'o' && identifier[1] == 'n' && identifier[2] == 'e') {
    return HEADER_VER1;
  }
  if (identifier[0] == 'b' && identifier[1] == 'i' && identifier[2] == 'g') {
    return HEADER_VER2;
  }
  ...
  if (header.resourceLength > 0) {
    ...
    if (resourcePtr != NULL) {
      ...
      if (!gFormatRecord->openForPreview) {
        ...
        CreateResourceInfoVector(header.resourceLength, reinterpret_cast<uint8 *>(resourcePtr), resources);

        // see if the user wants to remove some resources
        if (showDialog && DoUI(resources)) {
          header.resourceLength = RemoveResources(header.resourceLength, resources, reinterpret_cast<uint8 *>(resourcePtr));
...
static void DoEstimateStart (void) {
  ...
  dataBytes = sizeof (FileHeader) + gFormatRecord->imageRsrcSize + RowBytes () * gFormatRecord->planes * imageSize.v;
...
static void LockHandles(void) {
  ...
  Boolean oldLock = FALSE;
  sPSHandle->SetLock(reinterpret_cast<Handle>(*gDataHandle), true, reinterpret_cast<Ptr *>(&gData), &oldLock);
...
static void UnlockHandles(void) {
  Boolean oldLock = FALSE;
  if (*gDataHandle) {
    sPSHandle->SetLock(reinterpret_cast<Handle>(*gDataHandle), false, reinterpret_cast<Ptr *>(&gData), &oldLock);
  }
}
...
static int32 RemoveResources(int32 resourceLength, const vector<ResourceInfo *> & rInfos, uint8 * resourcePtr) {
  size_t count = rInfos.size();

  for (size_t a = 0; a < count; a++) {
    if (! rInfos.at(a)->keep) {
      resourceLength = RemoveResource(rInfos.at(a)->type, rInfos.at(a)->id, resourcePtr, resourceLength, resourcePtr);
    }
  }
