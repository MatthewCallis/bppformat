...
class BPPFormatDialog : public PIDialog {
private:
...
public:
  BPPFormatDialog(vector<ResourceInfo *> rInfos) : PIDialog(),
                  resourceList(),
                  resourceType(),
                  resourceID(),
                  resourceName(),
...
void BPPFormatDialog::Init(void) {
  PIItem item;
...
void BPPFormatDialog::Notify(int32 item) {
...
void BPPFormatDialog::GetResourceNames(vector<ResourceInfo *> & resourceInfos) {
  size_t count = resourceInfos.size();
  if (count == 0) {
    return;
  }
